...
pub struct Span {
    /// The starting position of the span in the source code.
    pub start: usize,
    /// The ending position of the span in the source code.
    pub end: usize,
}
...
pub enum ImportTreeKind {
    ...
    Glob,
    /// Represents a list of import trees.
    List(Vec<ImportTree>),
}
...
/// A type alias item, representing a named alias for a type.
#[derive(Debug, Clone)]
pub struct TypeAlias {
    ...
    /// The aliased type.
    pub ty: Type,
    /// The span of the type alias in the source code.
    pub span: Span,
}
...
pub struct LambdaArg {
    /// The pattern that matches the argument.
    pub pattern: Pattern,
